...
class General(Cog, description="General commands of the bot."):
    ...
    async def serverinfo(self, ctx: Context):
        days_ago = (datetime.today().replace(tzinfo=ctx.guild.created_at.tzinfo) - ctx.guild.created_at).days
        mfa = "False" if ctx.guild.mfa_level == 0 else "True"
        description = ctx.guild.description if ctx.guild.description is not None else "No description set."
        serverinfo = Embed(title=f"{ctx.guild.name} || ID: {ctx.guild.id}", description=f"{description}", color=self.bot.main_color)
        ...
        serverinfo.add_field(
            name="Miscellaneous", value=f"```Large Server: {ctx.guild.large}\nVerification level: {ctx.guild.verification_level}\n2FA: {mfa}```", inline=False
        )
    ...
    @command(name="stats", aliases=["about", "info"], brief="Shows a bunch of stuff about the bot.")
    async def stats(self, ctx: Context) -> None:
        stats_embed = Embed(title=f"Bot stats | Commands: {len(self.bot.commands)}", color=self.bot.main_color)
    ...
    async def weather(self, ctx: Context, *, city):
        ...
        try:
            ...
            return await ctx.send("I don't rember this town, perhaps specify the country..?")
        ...
        weather_embed.set_footer(text=f"Last updated: {util.format_datetime_from_string(current['last_updated'])}")
        ...
        weather_embed.add_field(name="Precipitation", value=f"{current['precip_mm']} mm | {current['precip_in']} inches")
        weather_embed.add_field(
            name="Wind",
            value=f"{current['wind_kph']} km/h ({current['wind_mph']} mp/h)\n" f"Heading: {current['wind_dir']} | {WIND_DIRECTIONS[current['wind_dir']]}",
        )
        weather_embed.add_field(name="Pressure", value=f"{current['pressure_mb']} mbar | {current['pressure_in']} inHg")
        ...
        weather_embed.add_field(name="Air quality", value=f"EPA-Standard: {US_EPA_INDEX.get(air_quality['us-epa-index'])}", inline=False)
    ...
    async def whatpulse(self, ctx: Context, username):
        ...
        whatpulse_embed = Embed(title=f"Whatpulse stats {data.get('AccountName')}", url=user_url, color=self.bot.main_color)
